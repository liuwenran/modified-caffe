...
template <typename Dtype>
class PQConvolutionLayer : public BaseConvolutionLayer<Dtype> {
 public:

  explicit PQConvolutionLayer(const LayerParameter& param)
      : BaseConvolutionLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual inline const char* type() const { return "PQConvolution"; }
 ...
 private:
  ...
  int iter_;
  int classnum_;
  float zeroThreshold_;
  Dtype* center_;
  int qstage_;
  std::vector<int> eachnum_;
  std::vector<Dtype> eachsum_;
};
