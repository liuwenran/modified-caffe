...
template <typename Dtype>
void PQConvolutionLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void PQConvolutionLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  const Dtype* weightTmp = this->weight_tmp_.gpu_data();  	
	// const Dtype* weightClass = this->blobs_[2]->gpu_data();
  ...
  for (int i = 0; i < top.size(); ++i) {    
    ...
    if (this->bias_term_ && this->param_propagate_down_[1]) {
			// const Dtype* biasClass = this->blobs_[3]->gpu_data();
...
INSTANTIATE_LAYER_GPU_FUNCS(PQConvolutionLayer);
